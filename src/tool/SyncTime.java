...
public class SyncTime {
	...
	final static String[] webUrl = { webUrl2, webUrl3, webUrl4, webUrl5, webUrl6, webUrl7, webUrl8 };
			...
			for (String url : webUrl) {
				...
				if (offset > 0) {
					...
					if (Math.abs(diff) < 60) {
						...
						System.out.println("Wait for network time sync, current: " + dateFormatTable.format(currentDate)
								+ ", network: " + dateFormatTable.format(networkDate));
						try {
							Thread.sleep(1000);
						} catch (InterruptedException e) {
							// TODO Auto-generated catch block
					...
					if(startOffset == 0) {
						...
						/* check overtime */
						if((offset - startOffset) > 30*60*1000){
